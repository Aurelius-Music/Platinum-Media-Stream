        ...
        <div style={styles.card}>
          ...
          {showHelp && (
            <div style={styles.helpBox}>
              ...
              <p style={styles.helpLine}><strong>3. Join as host</strong> — check this if you're starting/running the room. Hosts can go live.</p>
              ...
              <p style={styles.helpLine}><strong>6. Go Live</strong> — once inside as host, tap "⚙ Stream settings" to pick a platform and paste your stream key, then tap Go Live.</p>
            </div>
          )}
...
function BroadcastControls({ roomName }) {
  ...
  const [rtmpUrl, setRtmpUrl] = useState(PLATFORM_PRESETS.facebook.defaultUrl);
  const [streamKey, setStreamKey] = useState('');

  const handlePlatformChange = (key) => {
    setPlatform(key);
    setRtmpUrl(PLATFORM_PRESETS[key].defaultUrl);
    setStreamKey('');
  };
  ...
  const goLive = async () => {
    ...
    if (!preset.supported) {
      setStatus(`${preset.label} isn't supported yet — see the note below.`);
      return;
    }
    ...
    <div style={styles.broadcastBar}>
      ...
      {showRtmpForm && !egressId && (
        <div style={styles.rtmpForm}>
          ...
          >
            {Object.entries(PLATFORM_PRESETS).map(([key, p]) => (
              <option key={key} value={key}>
                {p.label}{!p.supported ? ' (not available yet)' : ''}
              </option>
            ))}
          </select>
          ...
          <p style={styles.rtmpHint}>{preset.note}</p>
        </div>
      )}
...
const styles = {
  ...
  rtmpHint: { color: '#999', fontSize: '10px', margin: 0, lineHeight: 1.4 },
};
